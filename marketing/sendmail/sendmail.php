...
if (isset($_POST['sendmail']) || $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($_POST['bot_detector'] == ''){
        ...
        $noofpages = isset($_POST['noofpages']) ? $_POST['noofpages'] : '-';
      ...
      // LEAD API END
        ...
        $data = '<h1 style="color: #01163A;text-align: center;">Authors Time Lead</h1><table style="width:100%; border: 1px solid black; border-collapse: collapse;">';
            $data .= '<tr>
    			...
    			<tr> 
    				<td style="border: 1px solid black; border-collapse: collapse; padding: 15px; text-align: left; font-size:15px; font-weight: 500;">No Of Pages<td>
    				<td style=" border: 1px solid black; border-collapse: collapse; padding: 15px; text-align: left; font-size:15px">' . $noofpages . '</td>
    			</tr>
    			<tr> 
      ...
      try {
        ...
        if ($mail->send()) {
          ...
          header("Location: /marketing/thankyou.php?name=$urlData");
        }
      } catch (Exception $e) {
